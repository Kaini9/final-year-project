                        ...
                        @forelse($posts as $post)
                            <div class="bg-white border border-gray-100 text-ink rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 post-item">
                                ...
                                @if($post->images && count($post->images) > 0)
                                    <div class="w-full bg-gray-100 border-y border-gray-100 relative" x-data="{ current: 0, images: {{ json_encode($post->images) }} }">
                                        <!-- Main Image -->
                                        <img :src="'{{ asset('storage') }}/' + images[current]" alt="Post image" class="w-full h-auto object-cover max-h-[850px] transition-opacity duration-300" loading="lazy">
                                        ...
                                        @foreach($post->comments->take(3) as $comment)
                                            <div class="text-[13px] flex gap-2 leading-tight items-start group">
                                                ...
                                                @if(Auth::id() === $comment->user_id || Auth::user()->isAdmin())
                                                    <form action="{{ route('comments.destroy', $comment) }}" method="POST" onsubmit="return confirm('Delete this comment?');" class="opacity-0 group-hover:opacity-100 transition-opacity">
                                                        @csrf
                                                        @method('DELETE')
    ...
    <script>
        ...
        if (loadMoreBtn) {
            loadMoreBtn.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch(`{{ route('feed.load-more') }}?page=${page}`);
        ...
        function createPostHTML(post) {
            const imageSlider = post.images.length > 1 ? `
                <div class="absolute top-3 right-3 bg-black/60 text-white px-3 py-1 rounded-full text-xs font-semibold">
                    1 / ${post.images.length}
                </div>
                <button type="button" class="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/80 hover:bg-white text-black flex items-center justify-center transition-all shadow-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                </button>
                <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/80 hover:bg-white text-black flex items-center justify-center transition-all shadow-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                </button>
            ` : '';

            return `<div class="post-item">Post loaded: ${post.id}</div>`;
        }
    </script>
</x-app-layout>
