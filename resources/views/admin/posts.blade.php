    ...
    <div class="bg-gray-50 min-h-screen py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="font-display text-4xl uppercase tracking-widest text-ink mb-1">Post Management</h1>
                    <p class="text-sm text-gray-500 font-medium">View and manage user posts.</p>
                </div>
            </div>
                ...
                <table class="w-full text-left text-sm text-gray-600">
                    <thead class="bg-gray-50 text-xs uppercase tracking-widest font-bold text-gray-500 border-b">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4">Likes / Comments</th>
                            <th scope="col" class="px-6 py-4">Posted Date</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($posts as $post)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-ink">{{ $post->user->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $post->user->email }}</div>
                                </td>
                                <td class="px-6 py-4 max-w-sm">
                                    <p class="truncate">{{ $post->caption }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    {{ $post->likes()->count() }} / {{ $post->comments()->count() }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-3" x-data="{ open: false }">
                                        <a href="{{ route('posts.show', $post) }}" target="_blank" class="text-xs font-bold uppercase tracking-widest text-indigo-600 hover:text-indigo-900 transition-colors">
                                            View
                                        </a>
                                                ...
                                                <div x-show="open" class="relative inline-block align-bottom bg-white border border-gray-200 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                                                    <form action="{{ route('admin.posts.destroy', $post) }}" method="POST">
                                                        ...
                                                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                            <h3 class="text-2xl font-display uppercase tracking-widest text-ink mb-1">Delete Post</h3>
                                                            <div class="mt-2">
